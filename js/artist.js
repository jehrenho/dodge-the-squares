...
export class Artist {
    ...
    static init(gameState, player, hazardManager, modifierManager) {
        ...
        Artist.ctx = canvas.getContext("2d");
        if (!Artist.ctx)
    ...
    static drawGameOver() {
        ...
        Artist.ctx.fillText(GAME_OVER_CONFIG.gameOverMessage.replace("{time}", Artist.gameState.getSecondsSurvived().toFixed(2)), GAME_CONFIG.VIRTUAL_WIDTH / 2, GAME_CONFIG.VIRTUAL_HEIGHT / 2 + 40);
